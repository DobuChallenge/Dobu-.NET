...
public sealed class AuthService(IUserStore users, string signingKey) : IAuthService
{
    ...
    public async Task<AuthResponse> RegisterAsync(RegisterRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var user = new Usuario(request.Nome, email, "temporary", request.TipoUsuario);
    ...
    public async Task<AuthResponse> LoginAsync(LoginRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (user is null || _hasher.VerifyHashedPassword(user, user.Senha, request.Senha) == PasswordVerificationResult.Failed)
